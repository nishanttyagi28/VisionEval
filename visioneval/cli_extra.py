"""Extra CLI commands: maps, traps gate, and run --use-budget wiring."""
...
def register_extra_commands() -> None:
    """Idempotent registration of maps / gate / use-budget onto the shared app."""

    # Avoid double-registration if imported twice
    ...
    trap_names = {cmd.name for cmd in traps_app.registered_commands}
    if "gate" not in trap_names:
