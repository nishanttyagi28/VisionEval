...
def write_reports(json_path: Path, markdown_path: Path | None, suite_name: str, summary: EvaluationSummary, regression: RegressionResult | None, partial: dict[str, object] | None = None) -> None:
    ...
    if markdown_path:
        ...
        lines = [f"# VisionEval: {suite_name}", "", f"- Status: **{status}**", f"- Accuracy: `{summary.accuracy:.4f}`", f"- Evaluated samples: `{len(summary.records)}`", f"- Prediction cache: `{summary.cache_hits}` hits / `{summary.cache_misses}` misses"]
        if partial:
        ...
        if regression:
            lines.extend([f"- Accuracy drop: `{regression.accuracy_drop:.4f}`", f"- New failures: `{len(regression.new_failures)}`", f"- Fixed failures: `{len(regression.fixed_failures)}`"])
